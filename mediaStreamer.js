...
wss.on('connection', function connection(ws) {
    ws.on('message', function incoming(str) {
        ...
        for (let i = 0; i < tracks.length ; i++) {
            promises.push(fetchSegmentBufferByUri(tracks[i]));
        }
 ...
 * @param {*} track - the track (init,tile or audio) 
 */
function fetchSegmentBufferByUri(track) {
    return new Promise(function (resolve, reject) {
        ...
        fetch(tileTrack.url)
          ...
          .then(function (buffer) {
            tileTrack.data = buffer;
            tileTrack.stats = {};
            tileTrack.stats.datalength = buffer.byteLength;
            tileTrack.stats.duration = Date.now() - beginTime;
            resolve(tileTrack);
          })
      })
    }
...
function HEVCMerger(split_video_data) {
    ...
    let length = split_video_data.reduce((sum, current) => {
        current.offset = 0;
        return sum + current.samples.reduce((sum, a) => sum + a, 0)
    }, 0)
    // console.log(length)
    let output = {
